...
| 6. | The date of birth (either 6 or 8 digits) must be a valid date | InvalidDateOfBirth |
...
The Luhn check digit is calculated using the 6 digits of the YYMMDD date of birth and the 3-digit birth serial number. If the value includes an 8-digit YYYYMMDD date of birth, the leading two digits are ignored.

The encoded date of birth may not be the person's actual date of birth. If the possible birth serial numbers for a particular date are exhausted, then a date close to the actual date of birth is used instead.
